...
def f_function(q):
    ...
    new_q1 = torch.sin(q1) * q2**2 + torch.exp(q3) - q4 * q5
    new_q2 = torch.cos(q2) + q3 * torch.tanh(q4) + q5**3
    scalar_output = (new_q1 + new_q2)**2 + torch.sqrt(torch.abs(new_q1 * new_q2))
...
def g_function(q):
    ...
    scalar_output = torch.exp(-norm_1) + torch.sqrt(norm_2 + 1)
    return scalar_output
