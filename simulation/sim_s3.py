...
def f_function(q):
    ...
    new_q1 = torch.tan(q1) + q1**2 * q2**2
    new_q2 = q2**3 - 2 * q1
    scalar_output = new_q1 * torch.sin(new_q2) + torch.sqrt(torch.abs(new_q1 - new_q2)) + 1 / (1 + new_q1**2)
    return scalar_output

def g_function(q):
    point = torch.tensor([0.0, 1.0])
    diff = q - point
    return torch.exp(-torch.norm(diff, p=1, dim=1))
